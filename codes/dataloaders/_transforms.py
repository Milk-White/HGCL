...
class RandomGenerator(object):
    ...
    def __call__(self, sample):
        ...
        # print(f"Before zoom - image shape: {image.shape}")
        
        x, y = image.shape[:2]
        # image = zoom(image, (self.output_size[0] / x, self.output_size[1] / y), order=0)
        # label = zoom(label, (self.output_size[0] / x, self.output_size[1] / y), order=0)

        # zoom_factors = tuple(self.output_size[i] / image.shape[i] for i in range(image.ndim))
        zoom_factors = tuple(self.output_size[i] / image.shape[i] for i in range(min(image.ndim, len(self.output_size))))

        # image = zoom(image, zoom_factors, order=0)
        # label = zoom(label, zoom_factors, order=0)
        image = np.resize(image, self.output_size)
        label = np.resize(label, self.output_size)


        # print(f"After zoom - image shape: {image.shape}")
...
class ToTensor:
  ...
  ####加的变换
    ...
    # 如果 img 还不是 Tensor，转换为 Tensor
        if isinstance(img, Image.Image):
            img = F.to_tensor(img)

    # 如果 label 还不是 Tensor，转换为 Tensor
...
class RandomCrop(T.RandomCrop):
    ...
    def forward(self, sample):
        ...
        # Assuming img_tensor is a 3D tensor with shape (C, H, W)
        img_array = img_tensor.permute(1, 2, 0).numpy().astype(np.uint8)
        width, height = Image.fromarray(img_array).size
