...
class UNetTF(BaseModel2D):
    def __init__(self,
        ...
        # 添加投影层 (使用1x1卷积代替线性层)
        self.project_layer = nn.Conv2d(in_channels=contrast_embed_dim, out_channels=128, kernel_size=1)  # 添加一个1x1卷积进行投影
    ...
    def _init_weights(self):
        ...
        kaiming_normal_init_weight(self.project_layer)  # 初始化投影层权重
    ...
    def forward(self, x, device):
        ...
        embedding = self.embed_head(features[self.contrast_embed_index]) if self.embed_head else None  # 如果 embed_head 存在提取特定层的特征并通过嵌入头生成嵌入向量 embedding
    ...
    def project(self, features):
        return self.project_layer(features)  # 使用1x1卷积层对特征进行处理

    def inference(self, x, **kwargs):
        features = self.encoder(x)  # 提取x的特征
        seg = self.seg_head(self.cnn_decoder(*features))  # 通过 CNN 解码器和分割头生成的结果
        preds = torch.argmax(seg, dim=1, keepdim=True).to(torch.float)  # 计算出每个像素的分割类别，并转换为浮点数格式
        return preds
